...
expected_report.conditions = {
    "reactor_volume/ uL": 411.0,
    "flow_profile_time/ s": np.array([0.0, 2.0, 4.0]),
    "Residence time/ s": np.array([120.0, 120.0, 120.0]),
    "O=C(CO)CO/ M": 0.4,
    "C=O/ M": 1.6,
    "[OH-]/ M": 0.12,
    "O/ M": 0.0,
    "O=C(CO)CO_flow_rate/ ul/h": np.array([1541.25, 1541.25, 1541.25]),
    "C=O_flow_rate/ ul/h": np.array([770.625, 770.625, 770.625]),
    "[OH-]_flow_rate/ ul/h": np.array([3082.5, 3082.5, 3082.5]),
    "CaCl2_flow_rate/ ul/h": np.array([3082.5, 3082.5, 3082.5]),
    "O_flow_rate/ ul/h": np.array([3853.125, 3853.125, 3853.125]),
}

expected_report.analysis_details = {
...
for c in expected_report.conditions:
    ...
    if c not in report.conditions:
        ...
        checks[name] = np.allclose(
            report.conditions[c], expected_report.conditions[c], atol=1e-8
        )
...
checks["series_values"] = np.allclose(
    report.series_values, expected_report.series_values, atol=1e-8
)
...
for e in expected_report.errors:
    ...
    if e not in report.errors:
        ...
        checks[name] = np.allclose(
            report.errors[d], expected_report.errors[d], atol=1e-8
        )
