...
def create_reaction_block(network, reaction):
    ...
    for reac_count, r_i in enumerate(reactant_indices):
        ...
        new_string = substitute_string(reaction_block[r_i][line_side], vline_token, vline_pos)
    ...
    for prod_count, p_i in enumerate(product_indices):
        ...
        new_string = substitute_string(reaction_block[p_i][line_side], vline_token, vline_pos)
    ...
    for reac_count, r_i in enumerate(reactant_indices):
        ...
        new_line = substitute_string(reaction_block[rxn_vpos][line_side],
                                    connect_token, vline_pos)
        reaction_block[rxn_vpos][line_side] = new_line

        if vline_pos < reaction_hpos:
            new_line = substitute_string(reaction_block[rxn_vpos][line_side],
                                        horizontal_line_token, vline_pos+1)
            reaction_block[rxn_vpos][line_side] = new_line
            new_line = substitute_string(reaction_block[rxn_vpos][line_side],
                                        horizontal_line_token, vline_pos+2)
            reaction_block[rxn_vpos][line_side] = new_line
        else:
            new_line = substitute_string(reaction_block[rxn_vpos][line_side],
                                        horizontal_line_token, vline_pos-1)
...
def reaction_score_layout(network):
    ...
    for c, reaction in enumerate(reactions):
        print(reaction)
        new_reaction_block = create_reaction_block(network, network.get_reaction(reaction))
...
network = file_import.load_network_from_reaction_list(reaction_list, name = '', description = '')
