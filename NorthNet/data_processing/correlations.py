...
def linkage_matrix(Euc_dist_mat):
...
def cluster_analysis(Euc_dist_mat, deps, graph_name):
    ...
    for x in range(0,len(deps)):
        la.append("{}".format(deps[x]))

    try:
        la =  [info_params.smiles_to_names[x] for x in la]
    except:
        la = la
    ...
    ax.set_xticklabels(la, fontsize = info_params.font, rotation = 30, ha = "right", va = "top", color= "k")
    dn = dendrogram(linkage_matrix, labels = la, leaf_font_size = info_params.font, leaf_rotation = 45, ax = ax)

    box = ax.get_position()
    ax.set_position([box.x0, box.y0 + box.height * 0.1, box.width, box.height * 0.9])
    #plt.savefig("{}_dendrogram.png".format(graph_name))
    #plt.clf()
    #plt.close()
